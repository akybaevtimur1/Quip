# CLAUDE.md — правила работы над ClipFlow / Quip

> 🚀 **НОВАЯ СЕССИЯ — читай `docs/README.md` ПЕРВЫМ** (единый индекс: что СЕЙЧАС в проде,
> порядок чтения, что НЕ ломать, карта деплоя). Затем — ЭТОТ файл (раздел «Железные правила»).
>
> ⚠️ **Реальность на 2026-06-15:** продукт ЖИВОЙ — фронт на Vercel `quip-app` (автодеплой на push
> в `main`), воркер на Modal `quip-worker`, Supabase + R2 + **биллинг (Polar) ВКЛЮЧЁН**. Журнал
> ниже и пометки «auth/оплата = Phase 1+, не трогай» (правило 7) — ИСТОРИЯ, всё это уже отгружено.
> `CLIPFLOW_DEV_PLAN.md` — историчен (Phase 0 завершён), не «единственный источник плана».
...
### Фидбек-фиксы фаундера (та же ветка `feat/production-shell`, 2026-06-13, вечер) — подтверждено фаундером
Вернулся с 3 проблемами, разобраны systematic-debugging (не угадывал):
- **Reframe «резко двигает камеру/флеши» (`ebfc3dc`):** диагноз — НЕ сломанный код (frame-accurate +
  настоящий ASD), а ИНХЕРЕНТНЫЙ дизайн: жёсткий сброс центра кропа на КАЖДОЙ склейке → телепорт на
  монтажном видео (сверил с SamurAIGPT — у них непрерывный EMA без склеек). Фикс **инвариант-безопасный**
  (REFRAME_FPS_GRID цел): центр едет НЕПРЕРЫВНО поперёк подряд идущих fill-шотов —
  `_track_trajectory(init_cx=prev_fill_end_cx)`; `plan_regions` тянет `prev_fill_end_cx` (сброс на
  fit/split). Границы режима fill/fit НЕ тронуты → флеши невозможны. +1 continuity-тест (84 reframe).
- **Редактор: превью кадра прыгает, на главной нет (`b571b55`):** ПОЧЕМУ — ДВА РАЗНЫХ плеера: главная
  играет ГОТОВЫЙ mp4 (reframe впечён ffmpeg'ом); редактор играет ИСХОДНИК + кропит НА ЛЕТУ в браузере
  (грубое приближение). Две причины, обе пофикшены: (1) `cxAt` была СТУПЕНЬКОЙ → линейная интерполяция;
  (2) кроп двигался только на `timeupdate` ~4Гц → CSS-transition object-position 300ms (fill+split) →
  плавный глайд. Превью = live (re-render не нужен для гладкости; continuity-регионы бакаются re-render'ом).
  ⚠️ **Находка:** `build_fill_crop_expr` в РЕНДЕРЕ тоже piecewise-CONST (ступенька ~3% шаги). Фаундер
  принял (мелко). Истинно линейный пан в скачанном файле → сделать expr piecewise-LINEAR (инвариант не
  трогает, только cx внутри fill-региона) — **follow-up по запросу фаундера.**
- **Editor «первая загрузка ошибка, потом retry работает» + 404 /terms /privacy:** 404 = устаревший
  prod-билд (в коде страницы ЕСТЬ). CORS/ERR_FAILED = воркер не успел подняться (torch/MediaPipe ~сек).
  Фикс: `ClipEditorScreen` авто-ретраит загрузку (4 раза, backoff) вместо мгновенной ошибки.
- **Дизайн (`2aef9f6`):** агент с дизайн-скиллами (frontend-design/ui-ux-pro-max/taste/design-review)
  собрал доступные примитивы **Checkbox/Switch/Select/IconButton** + применил по гриду/карточкам/табам
  (голые чекбоксы → стилизованные, кнопки-ссылки → настоящие кнопки; Button +`accent` коралл-вариант).
  Доделал HookTab + починил SSR-варнинг `recent.ts` (getServerSnapshot стабильный `[]`). Lighthouse
  лендинга 100×4 цел.
- ⚠️ **Воркер перезапущен мной** (новый reframe-код, :8000). Кэш `data/<job>/analysis/acc_*.json` для
