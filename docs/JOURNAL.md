...
- **#5 (КРИТ, регрессия #1):** force-framing одного шота превращал ВЕСЬ клип в этот режим и стирал
  ...
  (midpoint+last-wins), СОХРАНЯЯ t0/t1 (инвариант цел — новых границ нет). Fast-path для override на
  весь интервал (таб «Кадр») оставлен. Чинит и превью, и рендер (общий путь). +тесты.
