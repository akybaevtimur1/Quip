...
### 2026-06-30 (perf, скачивание) — composite-ASS: «скачать с субтитрами» 7.4× быстрее + фикс «тёмного первого файла»
Базовый клип рендерится БЕЗ субтитров (`run.py` зовёт `render_clip` без `ass_name`) — субтитры это libass-оверлей в браузере. Поэтому каждое «скачать с субтитрами» гнало ВЕСЬ пайплайн от источника (download R2 → PySceneDetect → MediaPipe ASD → полный libx264) на лёгком web-контейнере = медленно + гонка холодного CV (битый/тёмный первый файл, H1). Фикс: новый `app/fast_export.py` — для caption/hook-only правок (один интервал 9:16, без crop-override, совпадает с границами клипа) `render_edit_to_file` накладывает ASS на УЖЕ готовый `clips/<id>.mp4` через `ffmpeg -vf subtitles -c:a copy` (без скачивания источника/CV/reframe). Гейт `can_composite_captions` явный + логируемый (нет тихого фолбэка); тримы/мульти-интервал/crop → полный путь. Замер: **240.5с → 32.4с (7.4×)**; в облаке больше (13MB клип vs 58MB источник). Cache-bust `?v=` теперь и на grid `video_url` (`db._resolve_clip_urls`, токен из `jobs.updated_at`) — добивает stale/partial Cloudflare-edge (H2). Multipart: порог 100→32MB, части 16MB, до 9000, `part_size` в ответе upload-url; клиент `UPLOAD_PART_CONCURRENCY` 3→6. UI: «Download all» = меню из 3 вариантов (с субтитрами / без / .SRT) вместо молчаливой отдачи клипов БЕЗ субтитров. grid-инвариант не тронут. Тесты: `test_fast_export.py`, `test_storage.py`, `test_db.py`, `lib/api.test.ts`.

### 2026-06-30 (фикс, шрифты) — казахские буквы на хуках/субтитрах + жирные кириллице-способные двойники (B2B Алматы)
Казахские буквы (ә ғ қ ң ө ұ ү һ + регистр) рисовались тофу (□): дефолт хука = Unbounded и пресет «u» = Russo One покрывают РУССКУЮ кириллицу, но НЕ казах-уникальные глифы, а гард их не свопал. Проверено fonttools: НИ ОДНА upstream-сборка Unbounded/Russo One этих глифов не содержит (замена самого TTF невозможна). Фикс-ядро: `KAZAKH_CODEPOINTS` (18) + `_has_kazakh` + `KAZAKH_INCOMPLETE_FONTS`; `resolve_font_for_text` свопает детерминированно (явный лог); зеркалится во фронт `assStyle.ts` → preview == render. Фикс-лук (выбор фаундера): `LOOK_MATCH_FOR_CYRILLIC` — на каждый из 7 display-слотов кириллице-способный жирный двойник (Unbounded→Rubik Black, Russo One→Play, Anton→Oswald Heavy, Bebas→Oswald, Archivo Black→Inter, Luckiest Guy→Nunito Black, Poppins→Rubik), каждый проверен fonttools на 18 глифов, применяется ко ВСЕЙ кириллице (русский тоже получает лук). 6 новых TTF в ОБОИХ каталогах (`services/worker/fonts` + `apps/web/public/libass/fonts`) + `LibassLayer`. Проофы: `.scratch/proofs/kazakh-font/`. Тесты: `test_captions_v2.py`, `test_fonts.py` (анти-тофу: каждый фолбэк обязан покрывать 18 глифов), `assStyle.test.ts`. Открытый вопрос фаундеру: 2 слабых лука (Archivo Black→Inter тонковат, Luckiest Guy→Nunito теряет комикс) — менять или оставить.

### 2026-06-30 (пропозал) — оптимизация рендера (только предложение, НЕ реализовано)
Разведка: «render latency» это прежде всего reframe-CV (MediaPipe+ASD+PySceneDetect ~2/3 на клип), энкод ~1/3. Самый дорогой «after-render» кусок (прожиг субтитров от источника) уже устранён в download-треке (composite-ASS). Остальные рычаги (audio `-c:a copy`, тюнинг пресета, дедуп декода [HIGH RISK grid], кэш источника на фан-аут, cold-start, NVENC=reject) с impact/risk/grid-флагом — `docs/archive/2026-06-30-render-optimization-proposal.md`. Реализация отложена по просьбе фаундера.

### 2026-06-30 (фича, i18n) — RU/EN тумблер интерфейса (Phase 0+1, core-фанел)
Основатель одобрил двуязычие и выбрал Phase 1 scope → ОТМЕНА прежнего правила «только английский» для core-фанела. Стек: **`next-intl` в COOKIE-режиме** (БЕЗ `[locale]`-URL и БЕЗ `proxy.ts` — чтобы не конфликтовать с SEO-роутами `/ru/use-case` и Supabase-редиректами). Локаль из cookie `NEXT_LOCALE` в `i18n/request.ts` (`getRequestConfig`); провайдер `NextIntlClientProvider` + динамический `<html lang>` через `getLocale()` в `app/layout.tsx`; плагин `createNextIntlPlugin` в `next.config.ts`. Каталоги auth/app — `messages/{en,ru}.json` (166 ключей, паритет; ICU-плюрализация RU one/few/many для «клипов»); копия лендинга — двуязычный `Record<Locale,…>` в `lib/landingContent.ts` (медиа клипов инвариантны, только текст per-locale, мерж в `getLandingContent`). Тумблер `EN / RU` — `components/i18n/LocaleSwitcher.tsx` (cookie 1y SameSite=Lax + `router.refresh()`), в лендинг-Nav, App-header, лендинг-Footer. Переведено: лендинг (`landingContent.ts` + все секции, server-секции async через `getLocale`, Nav/Faq client через `useLocale`), auth (`AuthForm`/login/signup/layout), дашборд + shell (`AppHeader`, `RecentProjects`, `UsageMeter`, `StatusBadge`, `JobProgress`, `SourceForm`, `ClipGrid`). Pure-хелпер `resolveLocale` (`i18n/locale.ts`) + TDD (`i18n/locale.test.ts`, 6 тестов). Побочный эффект cookie-режима: marketing-страницы → динамический рендер (краулеры без cookie → дефолт `en`, hreflang/og не тронуты). ОТЛОЖЕНО (ещё английское): редактор + `ExportMenu`, worker-side `JobError`. Верификация: `tsc --noEmit` чисто, `vitest` 86/86, `next build` зелёный (38 страниц). Доки обновлены: `CLAUDE.md` §«Язык интерфейса» + `docs/README.md`.

### 2026-06-30 (perf, аплоад) — транскрипция стартует СРАЗУ: аудио из СЫРОГО файла, _ensure_mp4 отложен
Фаундерский приоритет #1 (латентность первой стадии аплоада). Раньше `import_upload` гнал `_ensure_mp4` (видео-нормализация: remux/полный libx264-ре-энкод) ДО `extract_audio` — транскрипция ждала ре-энкод (для несовместимого видео-кодека это ~8.6 с/мин source → 60-мин аплоад ≈ минуты скрытого ожидания). Stage 0 разрезан на две части: `prepare_upload_audio` (probe + meta.json + `extract_audio` ИЗ СЫРОГО файла — транскрипция стартует ≈мгновенно) и `normalize_upload_source` (отложенный `_ensure_mp4`). `run_pipeline` получил `normalize_source`-callback: материализует `source.mp4` ПОСЛЕ select, ПЕРЕД заливкой в R2 / per-clip фан-аутом (где впервые нужен seekable mp4). Параметры `_ensure_mp4` НЕ изменены → качество клипа и кадровая сетка идентичны (двигается ТОЛЬКО момент). meta probe'ится из сырого исходника (ре-энкод не масштабирует → fps/размеры совпадают). Замер на AV1-сэмпле (`job_b530fc7f5488`): time-to-transcription-start 71.56с (worst-case full re-encode) → 0.44с (164×); для уже-remux'абельного — ~0.6→0.44с. `stage3/stage5` не тронуты. Тесты: `tests/unit/test_upload_defer.py` (порядок + контракт «mp4 валиден до R2/фан-аута» с моками ffmpeg). 1081 pytest зелёный.
