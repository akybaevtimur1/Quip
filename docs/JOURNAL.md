...
### 2026-06-22 — Вкладка «Shots»: пер-шот кадрирование вынесено в свою rail-вкладку (коммит 7c4508f)
Пер-шот реврейм (`FitTimeline`) был закопан внизу вкладки Frame → вынесен в ОТДЕЛЬНУЮ вкладку рейла
«Shots». ⚠️ **Собрано + `just check` зелёный, но НЕ ЗАДЕПЛОЕНО** — ждёт верификации фаундера на
localhost:3000, затем сигнала на деплой. Файлы: `apps/web/components/editor/EditorRail.tsx`,
`ClipEditorScreen.tsx`, `EditorCanvas.tsx`, `FitTimeline.tsx`.
