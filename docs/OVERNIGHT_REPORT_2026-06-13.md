...
## 8. Пост-фидбек фиксы (живой тест фаундера, та же ветка, коммиты `3d41ca2`…`c2eb755`)

Фаундер тестил через UI на **29.97fps** видео. Сначала «ничего не работало» — **корень:
воркер крутил старый код** (uvicorn без --reload). Перезапустил воркер+фронт, проверил
HTTP-раундтрипом. Затем 4 бага (systematic-debugging, не угадывал):

- **#2 аспект ломал страницу** (`3d41ca2`): PreviewPlayer хардкодил 9:16; 16:9 распирал
  пол-страницы. Фикс: PreviewPlayer сам contain'ится по aspectClass.
- **#3 рывки/флеши наведения В РЕДАКТОРЕ** (`3d41ca2`): editor-путь reframe был отдельным
  инфериором (cuts в секундах + 5fps без ASD) → на ≠25fps флеши. Перевёл на ЕДИНЫЙ batch
  `reframe_segment` (frame-accurate + ASD). DoD на реальном 29.97fps: MAX Δ=0.00000.
- **#1 live-vs-рендер** (`0f0c9bd`): персистентный хинт + всегда видимый dirty-чип.
- **#4 подсветка убогая** (`9a4c41d`): по OSS-ресёрчу — вирусный стандарт = активное слово
  вспыхивает + pop. Сделал дефолтом (preset_a animation=pop). Статичный keyword → опция.
- **T4 #8** (двойные субтитры) добито: render тоже уважает `captions.burn`.

Гейт: `just check` зелёный (391 тест), `next build` зелёный. Урок фаундера зафиксирован:
**OSS-first** (память feedback-oss-first). Оба сервера перезапущены на новом коде.
