...
> ⚠️ **УСТАРЕЛО.** Supabase уже подключён (проект `qiagetbnsssvbiowuxpp`, миграции 0001–0005),
> биллинг ЖИВОЙ через **Polar** (не Lemon Squeezy), прайсинг = кредит-модель ($10 Starter / 10),
> а НЕ старые «$12 / 20 видео». Не следуй этому файлу для проводки оплаты — см. `docs/README.md`
> и `services/worker/app/billing.py`. Оставлен как история.
