...
class TestResolveRenderPolicy:
    ...
    def test_pro_clean_1080(self) -> None:
        p = resolve_render_policy("pro", local_dev=False)
        assert p.watermark is False
        assert p.max_resolution == 1080

    def test_unknown_plan_defaults_to_free_watermark(self) -> None:
    ...
    def test_local_dev_ignores_plan_watermark(self) -> None:
        # Даже если plan=free, local_dev гасит вотермарку (cloud-only enforcement).
